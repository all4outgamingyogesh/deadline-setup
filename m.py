...
async def add_user(message: Message):
    if message.from_user.id!= ADMIN_ID:
        await message.answer(" Are you admin ? I think You are not admin ? 😒")
        return
...
async def user_info(message: Message):
    ...
    if user_data:
        ...
        approval_expiry_str = "𝙊𝙥𝙥𝙨 𝙉𝙤𝙩 𝙖𝙥𝙥𝙧𝙤𝙫𝙚𝙙 𝘾𝙤𝙣𝙩𝙖𝙘𝙩 @valkyraegfx"
...
async def welcome_user(message: Message):
    if not await check_authorization(message.from_user.id):
        await message.answer("𝘼𝙘𝙘𝙚𝙨𝙨 𝙙𝙚𝙣𝙞𝙚𝙙\n 𝙔𝙤𝙪 𝙖𝙧𝙚 𝙣𝙤𝙩 𝙖𝙪𝙩𝙝𝙤𝙧𝙞𝙯𝙚𝙙 𝙩𝙤 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝙗𝙤𝙩\n 𝙠𝙞𝙣𝙙𝙡𝙮 𝘿𝙢 @valkyraegfx 𝙏𝙤 𝙂𝙚𝙩 𝘼𝙘𝙘𝙚𝙨𝙨")
        return

    await message.answer(f"𝙒𝙚𝙡𝙘𝙤𝙢𝙚 𝙩𝙤 𝘿𝙀𝘼𝘿𝙇𝙄𝙉𝙀 𝘿𝘿𝙊𝙎 𝘽𝙤𝙩! 🚀\n\n"
